...
int lead_time_check()
{
  int curr_time; // To keep time results
  TMR0 = 2; // Initialise TIMER0 to 2 (as 2 cycles will be left out)
  if(!RA3) // if the lead in is still active
  {    
    while(1)
    {
      if(RA3)
  ...
  if (MIN_LEAD < curr_time && curr_time < MAX_LEAD)
  {
    TMR0 = 2; // Reset TIMER0
    T0IF = 0; // reset overflow bit
    while(1)
    {
      if(!RA3) // if the next bit starts, check time interval of lead-in space...
      {
	curr_time = TMR0;
	break;
      }
    ...
    if(MIN_LEAD_SPACE < curr_time && curr_time < MAX_LEAD_SPACE)
    {
      return 1;
    }
    else
